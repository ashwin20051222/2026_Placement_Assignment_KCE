...
/*
 * Program 25: Home appliance control using a small networking example.
...
int main(void) {
    ...
    if (appliance_pid == 0) {
        struct ApplianceCommand command;
        ...
        while (read(control_pipe[0], &command, sizeof(command)) == (ssize_t)sizeof(command)) {
            ...
            printf("Home device process: applying command -> %s\n", command.command);
        }
...

    {
        ...
        while (fgets(line, sizeof(line), stream) != NULL) {
            ...
            printf("Receiver process: network command -> %s\n", command.command);
